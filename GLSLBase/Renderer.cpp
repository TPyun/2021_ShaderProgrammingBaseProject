...
void Renderer::CreateVertexBufferObjects()
{
		...
		=
	{
		-0.5, -0.5, 0.f,
		-0.5, 0.5, 0.f,
		0.5, 0.5, 0.f,		 //Triangle1
		-0.5, -0.5, 0.f,
		0.5, 0.5, 0.f,
		0.5, -0.5, 0.f,		 //Triangle2
	};
		...
		=
	{
		-0.5, -0.5, 0.f, 1, 0, 0, 1,
		-0.5, 0.5, 0.f, 0, 1, 0, 1,
		0.5, 0.5, 0.f, 0, 0, 1, 1,		 //Triangle1
		- 0.5, -0.5, 0.f, 1, 0, 0, 1,
		0.5, 0.5, 0.f, 0, 0, 1, 1,
		0.5, -0.5, 0.f, 0, 0, 1, 1,		 //Triangle2
	};
...
void Renderer::CreateParticle(int count)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		randomValueZ = 0.f;
		randomValueVX = ((float)rand() / (float)RAND_MAX - 0.5f) * 0.5f; //-1~1
		randomValueVY = ((float)rand() / (float)RAND_MAX - 0.5f) * 0.5f; //-1~1
		randomValueVZ = 0.f;
		randomEmitTime = ((float)rand() / (float)RAND_MAX) * 1.f;
		...
		g = ((float)rand() / (float)RAND_MAX) * 0.6f;
		b = ((float)rand() / (float)RAND_MAX) * 0.8f;
		a = 1.f; // ((float)rand() / (float)RAND_MAX) * 1.f;
		//v0
...
void Renderer::Lecture3()
{
	...
	glUniform4f(uniformLocColor, 1, 1, 0, 1);
...
void Renderer::Lecture3_Particle()
{
	...
	int attribRaondomValue = glGetAttribLocation(shader, "a_RandomValue");
	glEnableVertexAttribArray(attribRaondomValue);
	glVertexAttribPointer(attribRaondomValue, 1, GL_FLOAT, GL_FALSE, sizeof(float) * 15, (GLvoid*)(sizeof(float) * 10));
